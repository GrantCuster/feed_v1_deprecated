...
export const makeR = (tick, l_width, l_height, center_x, center_y) => {
  const duration = 200;
  ...
  l.cx = l_width * 3 / 4;
  l.ay = l.by = 0;
  l.dy = l.cy = l_height / 2;
  l.ey = l.fy = l_height;

  if (key0) {
    l.ax = l.dx = l.ex = key0 * l_width / 2;
    l.bx = l.fx = l_width - l.ax;
    l.cx = l_width * 3 / 4 - key0 * l_width / 4;
  } else if (key1) {
    const angle = key1 * 90;
    ...
    l.ay = l.by = l_height / 2 - key2 * l_height / 2;
    l.ey = l.fy = l_height / 2 + key2 * l_height / 2;
    l.cx = l_width - key2 * l_width / 4;
  }
