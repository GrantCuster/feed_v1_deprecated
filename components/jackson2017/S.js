...
export const makeS = (tick, l_width, l_height, center_x, center_y) => {
  const duration = 200;
  ...
  l.by = 0;
  l.ay = l_height / 4;
  l.cy = l.dy = l_height / 2;
  l.fy = l_height * 3 / 4;
  l.ey = l_height;

  if (key0) {
    l.cx = l.fx = key0 * l_width / 2;
    l.ax = l.dx = l_width - l.cx;
    l.ay = l_height / 4 + key0 * l_height / 4;
    l.fy = l_height * 3 / 4 - key0 * l_height / 4;
  } else if (key1) {
    const angle = key1 * 90;
    ...
    l.dx = l.ex = l.fx  = post_bot.x;
    l.dy = l.ey = l.fy  = post_bot.y;
  } else if (key2) {
    l.by = l_height / 2 - key2 * l_height / 2;
    l.ay = l_height / 2 - key2 * l_height / 4;
    l.fy = l_height / 2 + key2 * l_height / 4;
    l.ey = l_height / 2 + key2 * l_height / 2;
  }
